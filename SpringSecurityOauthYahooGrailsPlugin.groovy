 ...
 */
class SpringSecurityOauthYahooGrailsPlugin {
    ...
    def title = 'Yahoo for Spring Security OAuth plugin'
    def author = 'Mihai Cazacu, Enrico Comiti, Alexey Zhokhov'
    def authorEmail = 'donbeave@gmail.com'
    def description = '''\
Integrate [Yahoo|http://www.yahoo.com] to [Spring Security OAuth plugin|http://grails.org/plugin/spring-security-oauth].
'''

    def documentation = 'https://github.com/donbeave/grails-spring-security-oauth-yahoo'
    ...
    def issueManagement = [system: 'GITHUB',
                           url   : 'https://github.com/donbeave/grails-spring-security-oauth-yahoo/issues']
    def scm = [url: 'https://github.com/donbeave/grails-spring-security-oauth-yahoo']
    ...
    private void loadConfig(ConfigObject config) {
        ...
        def newConfig = new ConfigSlurper(Environment.current.name).parse(
                classLoader.loadClass('DefaultYahooOauthConfig')
        )

        newConfig.oauth.providers.yahoo.merge(config.oauth.providers.yahoo)

        // Now merge DefaultYahooOauthConfig into the main config
